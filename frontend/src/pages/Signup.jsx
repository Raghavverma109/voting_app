...
/>
  ...
  onResult={(result, error) => {
    if (result) {
      handleScan(result?.text); // ✅ handle valid QR code
    }
    if (error) {
      // Only log serious errors (optional)
      if (
        error.name !== "NotFoundException" && // frame without QR
        error.name !== "ChecksumException" && // QR partially read
        error.name !== "FormatException"      // QR partially corrupted
      ) {
